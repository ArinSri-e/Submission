    ...
    def solve(
        ...
        **solver_params
        ...
        cons_fixed = [
            ...
            T   >= 0, T   <= 1.0,
            M   >= 0, M   <= 1.0,
            cp.log1p(T) >= t_log,  cp.sqrt(M) >= m_sqrt
        ...
        for it in range(max_iter):
            ...
            r_d_cp   = r[self.dest_map]
            xgap_aff = r_d_cp - cp.mean(r_d_cp)
            dr_aff   = r_d_cp - self.prev_rates[self.dest_map]
            ...
            if hasattr(self, "coef_rate_lin"):
                # LINEAR MODE (affine in r): uses arrays you inject from D2 betas
                r_d_cp = r[self.dest_map]
                xg_np  = y0_np - y0_np.mean()  # already computed above
                Pflat = (
                    cp.Constant(self.intercept_flat)
                + cp.multiply(cp.Constant(self.coef_rate_lin),  r_d_cp)
                + cp.multiply(cp.Constant(self.coef_xgap_flat), xg_np)
                )
            else:
                ...
                Pflat = (
                    self.intercept_adj_flat
                + cp.multiply(self.coef_rcsq, rcsq_aff)
                + cp.multiply(self.coef_xgap,  xgap_aff)
                + cp.multiply(self.coef_drate, dr_aff)
                + cp.multiply(self.coef_promo, BPO)
                + cp.multiply(self.coef_logT,  t_log)
                + cp.multiply(self.coef_sqrt,  m_sqrt)
                )
            ...
            # if tr_radius is not None and idx_int.size > 0:
